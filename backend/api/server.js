...
const { registrationValidation, sendResetPasswordSMS } = require('../api/services/services.js');
...
fastify.post('/Authorization', async (req, reply) => {
  try {
    ...
    reply.header('Set-Cookie', `auth_token=${token}; HttpOnly; Secure; Max-Age=3600`);
...
fastify.post('/Registration', async (req, reply) => {
  try {
    ...
    return reply.status(500).send({ error: 'Ошибки при создании пользователя' });
  }
});
...
fastify.post('/reset-password', async (req, res) => {
  ...
  try {
    ...
    res.send('Password reset instructions have been sent to your phone number.');
